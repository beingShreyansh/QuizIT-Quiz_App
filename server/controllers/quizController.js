...
const deleteQuiz = (req, res) => {
    ...
    const identifyUserHistoryEntries = () => {
        ...
        pool.query(query, [quizId], (error, results) => {
            if (error) {
                ...
                res.status(500).json({ error: 'An error occurred while identifying user history entries.' });
            } else {
                const missingQuizIds = results.map(result => result.quiz_id);
                console.log('User history entries related to the quiz to be deleted:', missingQuizIds);
    ...
    const deleteQuizEntries = () => {
        ...
        const deleteQuizQuestionsQuery = 'DELETE FROM quiz_question WHERE quiz_id = ?';
        ...
        pool.query(deleteOptionsQuery, [quizId], (optionsError, optionsResults) => {
            if (optionsError) {
                ...
                res.status(500).json({ error: 'An error occurred while deleting quiz options.' });
                return;
            }

            pool.query(deleteQuizQuestionsQuery, [quizId], (questionsError, questionsResults) => {
                if (questionsError) {
                    console.error('Error deleting quiz questions:', questionsError);
                    res.status(500).json({ error: 'An error occurred while deleting quiz questions.' });
                    return;
                }
...
const updateQuestion = (req, res) => {
    ...
    const { questionContent, quizId, proficiencyLevel, questionType, option_1, option_2, option_3, option_4, option_5 } = req.body;
    ...
    `;
    let queryParams = [questionContent, proficiencyLevel, questionType, questionId, quizId];
    ...
    pool.query(query, queryParams, (error, results) => {
        if (error) {
            ...
            if (option_1 || option_2 || option_3 || option_4 || option_5) {
                ...
                pool.query(optionIdQuery, optionIdQueryParams, (optionIdError, optionIdResults) => {
                    if (optionIdError) {
                        ...
                        let optionsQuery = `
                            ...
                            WHERE question_id = ?;
                        `;
                        let optionsQueryParams = [optionIds[0], option_1, optionIds[1], option_2, optionIds[2], option_3, optionIds[3], option_4, optionIds[4], option_5, questionId];
                        // Execute the query to update options
                        pool.query(optionsQuery, optionsQueryParams, (optionsError, optionsResults) => {
                            if (optionsError) {
...
const deleteQuestion = (req, res) => {
    ...
    pool.query(deleteOptionsQuery, [questionId], (optionsError, optionsResults) => {
        if (optionsError) {
            console.error('Error deleting options:', optionsError);
        ...
        const deleteQuestionQuery = 'DELETE FROM quiz_question WHERE question_id= ?';
        pool.query(deleteQuestionQuery, [questionId], (questionError, questionResults) => {
            if (questionError) {
...
module.exports = { fetchCategories, fetchQuestionsAndOptions,deleteQuiz ,updateQuestion,deleteQuestion };